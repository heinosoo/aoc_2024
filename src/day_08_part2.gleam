...
const test_cases = [
  TestCase("inputs/day_08/test1.txt", "34"),
  TestCase("inputs/day_08/input.txt", "1246"),
]
...
fn add_antinodes(corner: #(Int, Int)) {
  fn(antinodes: set.Set(#(Int, Int)), antenna_pair: #(#(Int, Int), #(Int, Int))) -> set.Set(
    ...
    let dx = antenna_pair.1.0 - antenna_pair.0.0
    let dy = antenna_pair.1.1 - antenna_pair.0.1
    let n = int.min(corner.0 / dx, corner.0 / dy)

    let x =
      list.range(-n, n)
      |> list.map(int.multiply(_, dx))
      |> list.map(int.add(_, antenna_pair.0.0))

    let y =
      list.range(-n, n)
      |> list.map(int.multiply(_, dy))
      |> list.map(int.add(_, antenna_pair.0.1))
